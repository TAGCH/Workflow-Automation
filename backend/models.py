...
class WorkflowImportedData(Base):
    __tablename__ = 'workflow_imported_data'
    ...
    def __repr__(self):
        return f"<WorkflowImportedData(id={self.id}, workflow_id={self.workflow_id})>"

# Back_populates to establish relationships
User.workflows = relationship("Workflow", order_by=Workflow.id, back_populates="owner")
Workflow.imported_data = relationship("WorkflowImportedData", order_by=WorkflowImportedData.id, back_populates="workflow")
